...
const BidRequests = () => {
  ...
  const fetchAllBids = async () => {
    const { data } = await axiosSecure.get(`/bids/${user?.email}?buyer=true`, {withCredentials: true})
    setBids(data);
  }
            ...
            <div className='overflow-hidden border border-gray-200  md:rounded-lg'>
              <table className='min-w-full divide-y divide-gray-200'>
                ...
                <tbody className='bg-white divide-y divide-gray-200 '>
                  {
                    bids.map(bid => <BidRequestTable 
                      key={bid._id} 
                      bid={bid}
                      handleStatusChange={handleStatusChange}></BidRequestTable>)
                  }
                </tbody>
              </table>
            </div>
